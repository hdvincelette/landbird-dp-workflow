...
loc.cols<- colnames(dplyr::select(Location, matches("location")))


for(a in 1:length(loc.cols)) {
  for (b in 1:nrow(Location)) {
    if (sf::st_is_empty(Location[b, loc.cols[a]]) == FALSE) {
      unnested.loc<- as.data.frame(sf::as_Spatial(Location[b, loc.cols[a]]))
      colname.loc<- colnames(unnested.loc)
      unnested.long<- unnested.loc[1]
      unnested.lat<- unnested.loc[2]
      if(!colnames(unnested.long) %in% colnames(Location)){
        Location %>%
          tibble::add_column(colname.loc[1] = NA,
                             .before = paste0(loc.cols[a]))
      }
      if(!colnames(unnested.lat) %in% colnames(Location)){
        
      }
      Location$colnames(unnested.long)[b] <- unnested.long
      Location$colnames(unnested.lat)[b] <- unnested.lat
    }
  }
}
...
for (i in 1:length(import2)){
   assign(names(import2[i]),as.data.frame(import2[[i]]))
}
