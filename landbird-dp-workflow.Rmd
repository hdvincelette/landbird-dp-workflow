    ...
    for (b in 1:nrow(domainItem_value.table)) {
      ...
      if (class(ref.dictionary) == "list") {
        ...
        if (length(domain.num) != 0) {
          domain.values <- unlist(sapply(temp[["dataDictionary"]][["domain"]][[domain.num]][["domainItem"]], "[", "value"))
        
          variable.def <- temp[["dataDictionary"]][["domain"]][[domain.num]][["description"]]
          }
        
      } else {
        domain.values <- input.dxnry %>%
      ...
      select.value.app(variable, variable.def, invalid.values)
      correct.value.app(input.data, invalid.values, domain.values)
