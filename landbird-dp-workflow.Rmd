...
if(any(c(sp.col, date.col) %in% unlist(purrr::map(raw.data.list, colnames)))){
  
# Make filter selections
species <- unique(unlist(sapply(sapply(raw.data.list, "[", sp.col))))
...
min.date<- min(do.call("c", lapply(
              date.ranges, "[[", "Min"
            )))
max.date<- min(do.call("c", lapply(
              date.ranges, "[[", "Min"
            )))


filter.df.app(species, min.date, max.date)
...
other.data.list <- raw.data.list %>%
  purrr::map(
    .f = ~ .x %>%
      ...
      plyr::mutate({{date.col}} := as.Date(.data[[date.col]], format = date.format))
    
    %>%
      dplyr::filter(
        is.na(.data[[date.col]]) |
          !data.table::between(.data[[date.col]], min(date.range), max(date.range), NAbounds =
                                 FALSE)
      )
  )
...
if (is.null(selected.df) == FALSE) {
  raw.data.list <-  c(list(merged_data = data.table::rbindlist(raw.data.list[names(raw.data.list) %in% selected.df], fill = TRUE)), raw.data.list[!names(raw.data.list) %in% selected.df])
}
...
for (a in 1:length(raw.data.list)) {
  ...
  # Check colnames
  invalid.colnames <- colnames(input.data)[!colnames(input.data) %in% dxnry.colnames]
  
  unused.colnames <- as.vector(dxnry.colnames[!dxnry.colnames %in% colnames(input.data)])
  
  
  if (length(invalid.colnames) != 0) {
    for (invalid.col in invalid.colnames) {
      correct.colname.app(input.data, invalid.col, unused.colnames)
      ...
      if (decision == "remove") {
        ...
        unused.colnames <- unused.colnames[!unused.colnames %in% col.choice]
