...
RDR.keyword <- "DOD_Shorebird_Migration"
...
if (auth.merge == TRUE) {
  ...
  select.import.app(
    dir = paste0(dirname(rstudioapi::documentPath()), "/downloads/data"),
    ext = c("txt", "csv", "xls*"),
    multiple = FALSE
  )
  ...
  comparison <- compareDF::compare_df(new, old, stop_on_error = FALSE)
...
# view<- raw.data.list[[1]]
# unique(view$SpeciesCode)
