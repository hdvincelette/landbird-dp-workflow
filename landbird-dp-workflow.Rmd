...
pacman::p_load("devtools", "move", "auk", "magrittr", "shiny")
...
template.list <- c("brood","nest","encounters")
...
if (length(filter.option) != 0) {
  if ("species" %in% filter.option) {
    ...
    if (length(sp.overlap) == 0) {
      ...
      message(cat(
        paste0(
          "The following species were detected in all imported files.\n"
        ),
  ...
  if ("date range" %in% filter.option) {
    ...
    while (is.na(start.date)) {
      start.date.entry <- as.character(svDialogs::dlg_input(message = "Start:")$res)
      if (length(start.date.entry) == 0) {
        break()
      } else if (start.date.entry == "") {
        start.date.entry <- NA
      }
    ...
    if (is.na(start.date) == FALSE) {
      end.date <- NA
      
      while (is.na(end.date)) {
        end.date.entry <- as.character(svDialogs::dlg_input(message = "End:")$res)
        if (length(end.date.entry) == 0) {
          break()
        } else if (end.date.entry == "") {
          end.date.entry <- NA
        }
      ...
      if (all(is.na(c(start.date, end.date))) == FALSE) {
        temp <- raw.data.list %>%
          purrr::map(.f = ~ .x %>%
                       dplyr::mutate({{date.col}} := as.Date(.data[[date.col]], format = date.format))
...
# Check if data frames should be combined
column.compare <- janitor::compare_df_cols(raw.data.list, bind_method  = "bind_rows") %>%
  dplyr::rowwise() %>%
  replace(is.na(.), "N/A") %>%
  ...
  #     huxtable::hux(column.compare) %>% huxtable::set_text_color(2:nrow(.), 2:ncol(.), "red"),
  #     colnames = FALSE
  #   ),
  #   cat(
  #     paste0(
  #       "\nThere are column name and/or class inconsistencies in the imported data frames.\n"
  #     )
  #   )
  # )
...
DT::datatable(
  column.compare,
  options = list(pageLength = nrow(column.compare), scrollX = '400px'),
  filter = 'top'
)


merge.choice <- utils::menu(c("please do", "absolutely not"),
                            graphics = TRUE,
                            title = cat(
                              paste0(
                                "\nVariable name and class inconsistencies between improted data frames are displayed in the Viewer pane.\nWould you like to combine these into a single data frame to enable faster data processing?\n"
...
if (merge.choice == 1) {
  raw.data.name <- names(raw.data.list)
  raw.data.list <- list(combined_data = dplyr::bind_rows(raw.data.list))
}

rstudioapi::executeCommand("viewerStop")
...
ui <- fluidPage(bootstrapPage(
  ...
  column(
    ...
    shinyWidgets::pickerInput(
      "species",
      h4(strong("Select one or more species.")),
      choices =  unique(unlist(sp.vectors)),
      selected = unique(unlist(sp.vectors)),
      multiple = TRUE,
    ...
    dateRangeInput(
      "date.range",
      h4(strong("Select a date range.")),
      start = min(do.call("c", lapply(
        date.ranges, "[[", "Min"
      ))),
      end = max(do.call("c", lapply(
        date.ranges, "[[", "Min"
      ))),
      min = min(do.call("c", lapply(
        date.ranges, "[[", "Min"
      ))),
      max = max(do.call("c", lapply(
        date.ranges, "[[", "Min"
      ))),
      format = "yyyy-mm-dd"
    ),
    ...
    hr(),
    br(),
    h4(strong("Number of selected rows")),
...
runGadget(ui,
          server,
          viewer = dialogViewer('landbird-dp-workflow ~ Filter data', height = 450))
...
for (a in 1:length(raw.data.list)) {
  ...
  if (class(ref.dictionary) == "list") {
    ...
    dxnry_colnames <- unique(input.dxnry$codeName)
    invalid_colnames <- colnames(input.data)[!colnames(input.data) %in% dxnry_colnames]
              ...
              while (reprompt == TRUE) {
                ...
                if (fix.choice == 1) {
                  if (class(ref.dictionary) == "list") {
                    ...
                    domain.num <- which(
                      sapply(temp[["dataDictionary"]][["domain"]], "[", "codeName") == domainItem_value.table$Variable[b]
                    )
                  ...
                  if (menu.skip == "N") {
                    ...
                    if (domainItem.num != 0) {
                      if (class(input.data[[domainItem_value.table$Variable[b]]]) != class(domainItems[domainItem.num]) &
                          class(input.data[[domainItem_value.table$Variable[b]]]) == "integer") {
                        if (is.na(as.numeric(domainItems[domainItem.num])) ==
                            FALSE) {
                          new.value <- as.numeric(domainItems[domainItem.num])
                        } else {
                          input.data <- input.data %>%
                            dplyr::mutate_at(domainItem_value.table$Variable[b],
                                             ~
                                               as.character(.))
                          
                          new.value <- domainItems[domainItem.num]
                        }
                      } else {
                        new.value <- domainItems[domainItem.num]
                      }
                      ...
                      input.data <- input.data %>%
                        dplyr::mutate_at(
                          domainItem_value.table$Variable[b],
                          ~ dplyr::case_when(
                            .x == domainItem.choice[c] ~ new.value,
                            TRUE ~ .x
                          )
                        )
...
if (auth.merge == TRUE) {
  ...
  test<- summary(arsenal::comparedf(
    dplyr::mutate(raw.data.list[["Eielson_Survey_v2"]], across(
      where(is.character), ~ dplyr::na_if(.x, "")
    )),
    dplyr::mutate(orig.data.list[["Eielson_Survey_v2"]], across(
      where(is.character), ~ dplyr::na_if(.x, "")
    ))
  ))
...
if(class(ref.dictionary) == "list") {
  ...
  if ("mdEditor file" %in% export.choice) {
    ...
    file.name <- strsplit(gsub("[[:punct:]]$", "", file.name), " +")[[1]]
...
if (filter.option == "by species") {
  sp.vectors <- sapply(sapply(data.list, "[", sp.col), unique)
