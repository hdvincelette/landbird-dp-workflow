...
rds.vector <- list.files(path = "data-required", pattern = ".rda")

for (a in 1:length(rds.vector)) {
  rds.file <- load(paste0("data-required/", rds.vector[a]))
  assign(rds.vector[a], rds.file)
}
...
for (a in 1:length(warning.list)) {
  ...
  dataType.table <-
    warn.table %>% dplyr::filter(Category == "dataType")
