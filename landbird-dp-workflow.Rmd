...
test<- rjson::toJSON(blankdomain)
blankjson[["data"]][[1]][["attributes"]][["json"]] <- blankdomain
...
for (a in 1:length(warning.list)) {
  ...
  CodeName.warn<- warn.tabe$Variable[warn.tabe$Category=="CodeName"]
  ...
  for(b in 1:length(CodeName.choice)){
    dictionarylist <-
      rjson::fromJSON(ref.dictionary[["data"]][[1]][["attributes"]][["json"]])
    
    dictionarylist[["dataDictionary"]][["entity"]][[1]][["attribute"]]
  }
  
  for (b in 1:nrow(warn.table)) {
  }

  }
  
  warning.list[[a]] <- warning.list[[a]]
}
