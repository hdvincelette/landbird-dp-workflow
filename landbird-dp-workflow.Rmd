...
utils::browseURL(data.download.path)
...
if (auth.merge == TRUE) {
  ...
  # Merge data
  new.data <- auth.data %>%
    merge(as.data.frame(add.data), all = TRUE, sort = FALSE) %>%
    dplyr::select(colnames(auth.data))
  
  
  #!!!# Evaluate changes
  test <- summary(arsenal::comparedf(
    auth.data %>% dplyr::mutate(across(
      where(is.character), ~ dplyr::na_if(.x, "")
    )),
    new.data %>% dplyr::mutate(across(
      where(is.character), ~ dplyr::na_if(.x, "")
    ))
  ))
