...
select.import.app(path = raw.data.path, multiple = TRUE)
...
data.filenames<- tools::file_path_sans_ext(selected.files)

select.import.app(path = dictionary.path, multiple = FALSE)

ref.dictionary <- importer(path = dictionary.path,
                            files = selected.files)[[1]]

dxnry.filename<- tools::file_path_sans_ext(selected.files)
...
for (a in 1:length(raw.data.list)) {
  ...
  if (nrow(domainItem_value.table) != 0) {
    for (b in 1:nrow(domainItem_value.table)) {
      ...
      add.value.app(variable, invalid.values, ref.dxnry)
