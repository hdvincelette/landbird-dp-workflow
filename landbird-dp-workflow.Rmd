...
if (endsWith(raw.data.path, "/")==FALSE) {
  raw.data.path <- paste0(raw.data.path, "/")
}
                  ...
                  if (menu.skip == "N") {
                    ...
                    if (domainItem.num != 0) {
                      ...
                      input.data <- input.data %>%
                        dplyr::mutate_at(
                          domainItem_value.table$Variable[b],
                          ~ dplyr::case_when(.x == domainItem.choice[c] ~ new.value, TRUE ~ .x)
                        )
...
for (x in 1:length(raw.data.list)) {
  ...
  ## Status codes
  # Add missing columns
  miss_cols <- setdiff(c(na.omit(capture_cols$my_col[!grepl("color", capture_cols$my_col)]), "TagType1", "TagType2", "TagStatus1", "TagStatus2"), colnames(input.data))
...
#!# Evaluate changes
# summary(arsenal::comparedf(dplyr::mutate(input.data,across(where(is.character), ~ dplyr::na_if(.x, ""))), dplyr::mutate(orig.data,across(where(is.character), ~ dplyr::na_if(.x, "")))))
...
if(dictionary.ext %in% c("json")) {
  export.choice <- utils::select.list(
   ...
   dxnry.name <- gsub("\\_mdeditor.*", "", dictionary.choice, ignore.case = TRUE)
  
   if ("tabular dictionary" %in% export.choice) {
     message(cat(
       paste0(
         "Provide a name for the tabular dictionary.\nThe current name is '",
         dxnry.name ,
   ...
   if ("mdjson dictionary" %in% export.choice) {
     ...
     oldsubject <-
       paste0('\"',
              'subject',
              '\":\"',
              gsub(".*\"subject\":\"(\\w+)\"*", "\\1", oldsubject),
              '\"')
     
     newsubject <-
       paste0('\"', 'subject', '\":[\"', 'tabularDataset', '\"]')
     
     gsub(".*\"subject\":\"(\\w+)\"*", "\\1", oldsubject)
     
     gsub('^(.\"subject\":{1})(.*)$', '\\1[\\2', oldsubject)
     
      gsub('(?<=\\\\"subject\":)(?=\\\\")', "[", oldsubject)
     
     newstring <- gsub('\"subject\":.*$\"', "[", input.dxnry[["data"]][[1]][["attributes"]][["json"]])
    ...
    input.dxnry[["data"]][[1]][["attributes"]][["json"]] <-
     ...
     test<- rjson::toJSON(x = input.dxnry)
   ...
   if ("mdEditor file" %in% export.choir) {
     ref.dictionary[["data"]][[record.num]] <- input.dxnry
   }
...
invalid.index <- which(cap_data$BBLStatus != cap_data$`temp:BBLStatus`)
...
if (length(invalid.index) != 0) {
  for (a in invalid.index) {
    message(cat(
      paste0(
        "The status for the following individual appears invalid.\nSpecies: ",
        cap_data$SpeciesCode[a],
        "\nBand number: ",
        cap_data$BandNumberOut[a],
        "\nStatus (reported): ",
        cap_data$BBLStatus[a],
        " [",
        bander_portal_lookups$description[grepl(stringr::str_sub(cap_data$BBLStatus[a], 1, 1),
                                                bander_portal_lookups$code) == TRUE &
                                            bander_portal_lookups$field == "Bird Status"],
        " ",
        bander_portal_lookups$description[grepl(stringr::str_sub(cap_data$BBLStatus[a], 2, 3),
                                                bander_portal_lookups$code) == TRUE &
                                            bander_portal_lookups$field == "Bird Status Extra Info"],
        "]",
        "\nStatus (detected): ",
        cap_data$`temp:BBLStatus`[a],
        " [",
        bander_portal_lookups$description[grepl(
          stringr::str_sub(cap_data$`temp:BBLStatus`[a], 1, 1),
          bander_portal_lookups$code
        ) == TRUE &
          bander_portal_lookups$field == "Bird Status"],
        " ",
        bander_portal_lookups$description[grepl(
          stringr::str_sub(cap_data$`temp:BBLStatus`[a], 2, 3),
          bander_portal_lookups$code
        ) == TRUE &
          bander_portal_lookups$field == "Bird Status Extra Info"],
        "]",
        ...
        cap_data$LROut[a],
        "\nTagStatus1: ",
        cap_data$TagStatus1[a],
        "\nTagStatus2: ",
        cap_data$TagStatus2[a]
      )
    ))
    ...
    if (status.choice == 1) {
      cap_data$BBLStatus[a] <- cap_data$`temp:BBLStatus`[a]
    ...
    if (status.choice == 2) {
      ...
      cap_data[match(invalid.entries.edit$OccurenceID,
                     cap_data$OccurenceID), "BBLStatus"] <-
        invalid.entries.edit$BBLStatus
    }
...
cap_data_aux <-
  cap_data %>% dplyr::filter(dplyr::if_any(leg_cols, ~ !. %in% c(NA, "00")))

cap_data_no_aux <-
  cap_data %>% dplyr::filter(dplyr::if_all(leg_cols, ~ . %in% c(NA, "00")))
...
for (a in c("cap_data_aux", "cap_data_no_aux")) {
  for (b in c("1", "R")) {
    if (!b %in% names(eval(as.name(a)))) {
      temp <-  list(rbind(tibble::as_tibble(
        matrix(
          ncol = length(cap_data),
          nrow = 1,
          dimnames = list(NULL, colnames(cap_data))
        ), check.names = FALSE
      )))
...
cap_data_no_aux$'1' <-
  tibble::as_tibble(data.table::rbindlist(cap_data_no_aux[c('1', '5', '8')]))
cap_data_no_aux <- suppressWarnings(within(cap_data_no_aux, rm('5','8')))
cap_data_aux$'1' <-
  tibble::as_tibble(data.table::rbindlist(cap_data_aux[c('1', '5', '8')]))
cap_data_aux <- suppressWarnings(within(cap_data_aux, rm('5', '8')))
...
```{r}
photo.file.path <- "C:/Users/hvincelette/OneDrive - DOI/Documents/Fieldwork_wrap/2024/Eielson/photos/ALMS_points/"

ref.file.path<- "C:/Users/hvincelette/OneDrive - DOI/Documents/Fieldwork_wrap/2024/Eielson/data/"
...
if (ref.data.ext %in% c("xlsx", "xls")) {
  ref.file <-
    readxl::read_excel(paste0(ref.file.path, ref.data.choice), na = "")
  
  ref.file <- ref.file %>%
    dplyr::mutate({{date.col}} := format(.data[[date.col]], format = date.format)) %>%
    dplyr::mutate({{time.col}} := format(.data[[time.col]], format = time.format))
  
} else if (ref.data.ext %in% c("csv")) {
  ref.file <-
    utils::read.csv(paste0(ref.file.path, ref.data.choice), na.strings = "")
