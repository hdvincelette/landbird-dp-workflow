...
for (a in 1:length(warning.list)) {
  ...
  # domainItem
  domainItem.table<- warn.table %>% dplyr::filter(Category=="domainItem_value")
  
  for(b in 1:nrow(domainItem.table)){
      ...
      utils::select.list(
        ...
        title = cat(
          paste0(
            ...
            import.data.name[a],
            "' contains entry values not found in the data dictionary. Select which, if any, to add to the domain item list in the data dictionary."
          )
        )
      )
      ...
      for (c in 1:length(domainItem.choice)) {
        new.dictionary <-
          modify.mdJSON(x = ref.dictionary,
                        ...
                        domainItem = domainItem.choice[c])
      }
